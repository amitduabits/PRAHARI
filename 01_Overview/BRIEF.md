...
**The Sentinel contract.** Every camera is a live RTP/RTSP stream. Catalogue `GET /cameras.json` (session cookie) is the source of truth. RTSP `rtsp://103.250.160.189:8554/stream/<id>` for inference. HLS `https://cctv.corp8.cloud/<id>/index.m3u8` for dashboards. TCP transport is mandatory. Timing is PTS. Feeds loop; expect a hard scene cut. There is no file download.
